...
void SysTick_Wait1s(unsigned long Multiplicand)
{
    char i;    //iteration
    for (i = 0; i <= Multiplicand; i++)
    {
        // 16 MHz precision internal oscillator -> 62.5 ns
        // 1s / 62.5 ns = 16,000,000 = 0x00F42400
        unsigned long delay_1s = 	0x00F42400;
        NVIC_ST_RELOAD_R = delay_1s - 0x00000001; //auto decrement
        // NVIC_ST_RELOAD_R = value to reload into CURRENT when 0 is reached
        NVIC_ST_CURRENT_R = 0; // initialize/clear 24bit current value of timer
        /*
        Systick Control = NVIC_ST_CTRL_R
        NVIC_ST_CTRL_R @ 0x00010000 = COUNT
        COUNT: 0 = timer did not reach - since last check, 
        1 = timer did reach - since last check
        */
        // while timer does not reach delay wait
